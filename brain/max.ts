...
export async function leadScorer(signals: Record<string, unknown>): Promise<number> {
  ...
  const out = await callModel({ tier: "max", system, user: JSON.stringify(signals), maxTokens: 8 });
...
export async function intentRouter(message: string): Promise<Segment> {
  const system =
    ...
    "החזר מילה אחת בלבד.";
  const out = (await callModel({ tier: "max", system, user: message, maxTokens: 4 })).toLowerCase();
