...
export const ELIGIBILITY_SECTIONS: EligibilitySection[] = [
  {
    ...
    body: [
      ...
      "לכן הכרטיסים שלמעלה מפרידים ביניהם ומקשרים לעמוד הזכאות הרשמי של כל אגף. שיעור הסבסוד המוכר וגובה המענק נקבעים ומתפרסמים על ידי האגף, וההכרעה בעניין אדם מסוים היא של האגף בלבד.",
      AUTHORITY_NOTE,
    ],
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      "אנחנו לא קובעים זכאות, לא מאשרים אותה, לא מייצגים אתכם מול משרד הביטחון ולא מבטיחים תוצאה. כל מספר שמופיע בדף הזה בהקשר של סבסוד או מענק הוא הכיסוי המרבי האפשרי, ולא התחייבות שהוא יחול עליכם. שיעור הסבסוד המוכר הוא זה שהאגפים קובעים ומפרסמים; ההשלמה שמעבר לו היא מענק רשות של היבואן ואינה חלק ממימון משרד הביטחון — כמפורט בהערה שמתחת לכרטיסים.",
    ],
  },
];
...
export const ELIGIBILITY_FAQ: EligibilityFaq[] = [
  ...
  },
  {
    ...
    a:
      "משרד הביטחון בלבד — אגף השיקום לנכי צה\"ל וכוחות הביטחון, אגף משפחות והנצחה לבני משפחות שכולות. MiaMe אינה קובעת זכאות, אינה מאשרת אותה ואינה צד להליך. שיעור הסבסוד המוכר נקבע ומתפרסם על ידי האגפים; ההשלמה שמעבר לו היא מענק רשות של היבואן, בכפוף לתנאיו וניתנת לשינוי או להפסקה. כל מספר שמוצג באתר בהקשר זה הוא הכיסוי המרבי האפשרי, ולא הבטחה אישית.",
  },
  {
  ...
  },
  {
    ...
    a:
      // The figures were spelled out here until 2026-09-10 — "סבסוד מוכר של עד 90% …
      // בתוספת מענק ההוקרה של MEU בשיעור 10%" — which broke this file's own rule at
      // the top ("One number, one home") in the worst possible place: this answer is
      // also emitted as FAQPage JSON-LD, so the frozen figures were being PUBLISHED to
      // Google from a second file. A fresh legal read that moved 90/10 in Tribute.tsx
      // would have left a stale pair in structured data. The worksheet renders on this
      // same page, centimetres away, carrying the numbers and the asterisk together.
      "זה תלוי במסלול ובהחלטת האגף, ולכן אין לזה מספר אחד. גיליון החישוב בדף הזה מראה את הפירוט המלא של הכיסוי המרבי האפשרי, ומסומן בכוכבית מסיבה זו — ההערה שמתחת לו מפרטת מה נכלל בו ומה תנאיו. " +
      AUTHORITY_NOTE +
